...
<head>
  ...
  <style>
    ...
    *{box-sizing:border-box}html,body{margin:0;padding:0}body{background:#e9f0f7;color:var(--ink);font-family:"Segoe UI",Tahoma,Arial,sans-serif;font-size:14px;line-height:1.45;font-kerning:normal;text-rendering:optimizeLegibility;-webkit-font-smoothing:antialiased;-webkit-print-color-adjust:exact;print-color-adjust:exact}
    ...
    .paper:before{content:"";position:absolute;right:-26mm;top:-17mm;width:104mm;height:62mm;border-radius:50%;background:radial-gradient(ellipse at center,#d9ecfb 0,#eef7ff 46%,#fff0 73%);transform:rotate(-9deg);pointer-events:none}.paper:after{content:"";position:absolute;right:-8mm;top:10mm;width:72mm;height:31mm;border-top:2px solid #d5eafb;border-radius:50%;transform:rotate(-10deg);pointer-events:none}
    .brand-row{position:relative;z-index:1;display:grid;grid-template-columns:minmax(0,1fr) 72mm;gap:8mm;align-items:center;padding-bottom:5mm;border-bottom:.45mm solid #6ea9da}.brand{display:flex;align-items:center;gap:5mm}.brand-logo{width:27mm;height:27mm;border:.35mm solid #86b8e4;border-radius:5mm;padding:1.8mm;background:#fff;display:flex;align-items:center;justify-content:center;box-shadow:0 2mm 5mm #13558a10}.brand-logo img{display:block;width:100%;height:100%;object-fit:contain}.brand-copy strong{display:block;color:var(--navy);font-size:26px;line-height:1.05;letter-spacing:.02em;font-weight:900}.brand-copy span{display:block;margin-top:2mm;color:#1d64a5;font-size:11px;font-weight:800;letter-spacing:.12em}
    ...
    .signature-row{display:grid;grid-template-columns:1fr 1fr;gap:34mm;margin-top:6mm;padding:0 14mm;text-align:center}.signature strong{display:block;color:#173f6c;font-size:15px;font-weight:900}.signature small{display:block;margin-top:1mm;color:#617d99;font-style:italic;font-size:11px}.signature-space{height:56mm}.signature-line{width:52mm;margin:0 auto;border-top:.4mm solid #4d7eaa}.signature-name{display:block;margin-top:2mm;color:#153c69;font-size:14px;font-weight:900}
    .footer{margin-top:5mm;display:flex;align-items:center;justify-content:center;gap:3mm;color:#2f6fa7;font-size:9px;font-weight:900;letter-spacing:.16em;text-align:center}.footer:before,.footer:after{content:"";width:25mm;border-top:.35mm solid #66a3d4}
...
<body>
  ...
  <main class="paper" id="receiptPaper">
    <header class="brand-row"><div class="brand"><div class="brand-logo"><img src="/logo-thay-dat-compact.webp?v=15" alt="Học lái xe cùng Đạt"></div><div class="brand-copy"><strong>HỌC LÁI XE CÙNG ĐẠT</strong><span>AN TOÀN HÔM NAY · VỮNG TAY TƯƠNG LAI</span></div></div><div class="contact" aria-label="Thông tin liên hệ"><div class="contact-row"><span class="contact-dot">☎</span><span>Hotline: <a href="tel:0984811037">0984811037</a></span></div><div class="contact-row"><span class="contact-dot">@</span><a href="mailto:thaydat.dtlx@gmail.com">thaydat.dtlx@gmail.com</a></div><div class="contact-row"><span class="contact-dot">W</span><a href="https://www.hoclaixecungdat.com/">www.hoclaixecungdat.com</a></div></div></header>
    <section class="title-block"><h1>BIÊN LAI HỌC PHÍ</h1><p>PHIẾU XÁC NHẬN THANH TOÁN HỌC PHÍ</p></section>
    <div class="meta-grid"><div class="meta-box"><span class="meta-icon">▣</span><span>Số phiếu:</span><strong>${escapeHtml(receiptNo)}</strong></div><div class="meta-box"><span class="meta-icon">▦</span><span>Ngày thu:</span><strong>${escapeHtml(date)}</strong></div></div>
    <section class="section"><div class="section-title"><span class="section-no">1</span>THÔNG TIN HỌC VIÊN</div><div class="student-card"><div class="student-grid"><div class="student-col"><div class="field"><span>Họ và tên:</span><strong>${escapeHtml(payment.student_name||"—")}</strong></div><div class="field"><span>Ngày sinh:</span><strong>${escapeHtml(birthDate)}</strong></div><div class="field"><span>Khóa học:</span><strong>${escapeHtml(courseDescription)}</strong></div><div class="field"><span>Địa chỉ:</span><strong>${escapeHtml(payment.address||"—")}</strong></div></div><div class="student-col"><div class="field"><span>Mã học viên:</span><strong>${escapeHtml(payment.student_code||"—")}</strong></div><div class="field"><span>Số CCCD:</span><strong>${escapeHtml(payment.cccd||"—")}</strong></div><div class="field"><span>Số điện thoại:</span><strong>${escapeHtml(payment.student_phone||"—")}</strong></div></div></div></div></section>
    <section class="section"><div class="section-title"><span class="section-no">2</span>CHI TIẾT THANH TOÁN</div><div class="payment-card"><table class="payment-table" aria-label="Chi tiết thanh toán học phí"><thead><tr><th class="stt">STT</th><th>Nội dung</th><th class="money">Số tiền</th></tr></thead><tbody><tr><td class="stt">1</td><td>Học phí khóa học</td><td class="money">${escapeHtml(displayMoney(tuitionTotal))}</td></tr><tr><td class="stt">2</td><td>Đóng học phí lần 1</td><td class="money">${escapeHtml(displayMoney(paidBefore))}</td></tr><tr><td class="stt">3</td><td>Thanh toán kỳ này</td><td class="money">${escapeHtml(amount)}</td></tr><tr><td class="stt">4</td><td>Còn lại</td><td class="money">${escapeHtml(displayMoney(debt))}</td></tr></tbody></table></div></section>
    <div class="detail-grid"><div class="detail-box"><div class="detail-icon">▰</div><div class="detail-copy"><span>Phương thức thanh toán:</span><strong>${escapeHtml(method)}</strong></div></div><div class="detail-box"><div class="detail-icon">▤</div><div class="detail-copy"><span>Nội dung thu:</span><strong>${escapeHtml(content)}</strong></div></div><div class="detail-box"><div class="detail-icon">₫</div><div class="detail-copy"><span>Số tiền bằng chữ:</span><strong class="words">${escapeHtml(amountWords)}</strong></div></div></div>
    <div class="total-strip ${isVoided?"is-voided":""}"><span class="coin">${isVoided?"!":"✓"}</span><b>${escapeHtml(totalLabel)}: ${escapeHtml(amount)}</b></div>${isVoided&&payment.void_reason?`<div class="void-reason">Lý do hủy: ${escapeHtml(payment.void_reason)}</div>`:""}
    <div class="signature-row"><div class="signature"><strong>Người nộp tiền</strong><small>(Ký và ghi rõ họ tên)</small><span class="signature-space"></span><div class="signature-line"></div><span class="signature-name">${escapeHtml(payment.student_name||"—")}</span></div><div class="signature"><strong>Người thu tiền</strong><small>(Ký và ghi rõ họ tên)</small><span class="signature-space"></span><div class="signature-line"></div><span class="signature-name">Trần Quốc Đạt</span></div></div>
  ...
  <script>
    ...
    const closeButton=document.getElementById("closeReceipt"),saveButton=document.getElementById("saveReceiptPng"),printButton=document.getElementById("printReceipt");
    closeButton.addEventListener("click",()=>window.close());printButton.addEventListener("click",()=>window.print());
    ...
    function loadHtml2Canvas(){if(window.html2canvas)return Promise.resolve(window.html2canvas);return new Promise((resolve,reject)=>{const script=document.createElement("script");script.src="https://cdn.jsdelivr.net/npm/html2canvas@1.4.1/dist/html2canvas.min.js";script.onload=()=>resolve(window.html2canvas);script.onerror=()=>reject(new Error("Không tải được trình xuất PNG"));document.head.appendChild(script)})}
    saveButton.addEventListener("click",async()=>{const original=saveButton.textContent;saveButton.disabled=true;saveButton.textContent="Đang lưu PNG…";try{const html2canvas=await loadHtml2Canvas();await Promise.all([...document.images].filter(image=>!image.complete).map(image=>new Promise(resolve=>{image.onload=image.onerror=resolve})));const canvas=await html2canvas(document.getElementById("receiptPaper"),{scale:2,backgroundColor:"#ffffff",useCORS:true,logging:false});const link=document.createElement("a");link.download=safeFileName("bien-lai-"+receiptNo)+".png";link.href=canvas.toDataURL("image/png");link.click()}catch(error){alert("Chưa thể lưu PNG. Bạn vẫn có thể dùng nút In / Lưu PDF A4.")}finally{saveButton.disabled=false;saveButton.textContent=original}});
  </script>
</body>
</html>`;
}
