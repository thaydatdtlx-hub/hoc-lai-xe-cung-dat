...
/* Legacy Cốc Cốc sidebar files remain only for compatibility/history. They must not be loaded by Admin. */
for(const required of [
  'import "./coccoc-sidebar.css"',
  'hoclaixecungdat_admin_sidebar_state_v1',
  'function bindSidebar(sidebar)',
  'coccoc-sidebar-tooltip'
])if(!coccocSidebar.includes(required))throw new Error(`Tệp tương thích thanh bên cũ thiếu: ${required}`);
for(const required of [
  '--coccoc-sidebar-collapsed-width:74px',
  '--coccoc-sidebar-expanded-width:248px',
  '.coccoc-sidebar-toggle',
  '.coccoc-sidebar-tooltip'
])if(!coccocSidebarStyles.includes(required))throw new Error(`CSS tương thích thanh bên cũ thiếu: ${required}`);

if(adminHome.includes('coccoc-sidebar.js'))throw new Error("Liên kết logo quản trị không được tải lại lớp sidebar cũ.");
...
console.log("Nâng cấp chuyên nghiệp hợp lệ: Admin chỉ dùng giao diện dashboard hiện tại; lớp sidebar/command center cũ không còn được nạp chồng lên giao diện mới.");