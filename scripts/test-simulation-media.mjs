...
for (const mutate of [
  d=>d.scenarios.push(scenario(1)),
  d=>d.scenarios[0].scoreWindows[1].start=4,
  d=>d.scenarios[0].durationSeconds=6,
  d=>d.scenarios[0].chapter=2,
  d=>d.scenarios[0].videoUrl='javascript:alert(1)',
  d=>delete d.scenarios[0].source
]) {
  const d=structuredClone(good);
  mutate(d);
  assert.ok(validateManifest(d).length);
}

const exam = makeExam(good.scenarios, () => 0.314159);
assert.equal(new Set(exam.map(s=>s.id)).size,10);
assert.deepEqual(
  Array.from({length:6},(_,i)=>exam.filter(s=>chapterOf(s.id)===i+1).length),
  EXAM_CHAPTER_COUNTS
);
assert.throws(()=>makeExam(good.scenarios.slice(0,119)));

const manifest=JSON.parse(await readFile(new URL('../public/data/mo-phong-120-media.json',import.meta.url)));
assert.deepEqual(validateManifest(manifest),[]);

if (manifest.version === 'pending') {
  assert.equal(manifest.scenarios.length,0);
  assert.ok(validateManifest(manifest,{requireComplete:true}).length);
  console.log('PASS: algorithm tests valid; pending release correctly blocked.');
} else {
  assert.equal(manifest.scenarios.length,120);
  assert.equal(new Set(manifest.scenarios.map(s=>s.id)).size,120);
  assert.deepEqual(validateManifest(manifest,{requireComplete:true}),[]);
  console.log('PASS: algorithm tests valid; production manifest complete at 120/120 scenarios.');
}
